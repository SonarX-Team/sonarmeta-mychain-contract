...
contract MRC998 is Ownable {
    ...
    constructor(identity _componentsidentity) public {
        components = MRC721(_componentsidentity);
    }

       /**
     * @dev See {IMRC998-balanceOf}.
     */
    function balanceOf(identity owner) public view returns (uint256) {
        require(owner != identity(0), "MRC998: identity zero is not a valid owner");
        return _balances[owner];
    }

    /**
     * @dev See {IMRC998-ownerOf}.
     */
    function ownerOf(uint256 tokenId) public view returns (identity) {
        identity owner = _ownerOf(tokenId);
        require(owner != identity(0), "MRC998: invalid token ID");
        return owner;
    }

    /**
     * @dev See {IMRC998-approve}.
     */
    function approve(identity to, uint256 tokenId) public {
        identity owner = MRC998.ownerOf(tokenId);
        require(to != owner, "MRC998: approval to current owner");

        require(
            msg.sender == owner || isApprovedForAll(owner, msg.sender),
            "MRC998: approve caller is not token owner or approved for all"
        );
    ...
    /**
     * @dev See {IMRC998-setApprovalForAll}.
     */
    ...
    /**
     * @dev See {IMRC998-isApprovedForAll}.
     */
    ...
    function transferFrom(
        ...
        require(_isApprovedOrOwner(msg.sender, tokenId), "MRC998: caller is not token owner or approved");

        _transfer(from, to, tokenId);
    }

    /**
     * @dev See {IMRC998-safeTransferFrom}.
     */
     ...
     * are aware of the MRC998 protocol to prevent tokens from being forever locked.
     *
     ...
     * - If `to` refers to a smart contract, it must implement {IMRC998Receiver-onMRC998Received}, which is called upon a safe transfer.
     *
    ...
    /**
     * @dev Same as {xref-MRC998-_safeMint-identity-uint256-}[`_safeMint`], with an additional `data` parameter which is
     * forwarded in {IMRC998Receiver-onMRC998Received} to contract recipients.
     */
    function _safeMint(
        identity to,
        uint256 tokenId
    ...
    function _mint(identity to, uint256 tokenId) internal {
        require(to != identity(0), "MRC998: mint to the zero identity");
        require(!_exists(tokenId), "MRC998: token already minted");
    ...
    function _transfer(
        ...
        require(ownerOf(tokenId) == from, "MRC998: transfer from incorrect owner");
        require(to != identity(0), "MRC998: transfer to the zero identity");
    ...
    function _approve(identity to, uint256 tokenId) internal {
        ...
        emit Approval(MRC998.ownerOf(tokenId), to, tokenId);
    }
    ...
    function _setApprovalForAll(
        ...
        require(owner != operator, "MRC998: approve to caller");
    ...
    function _requireMinted(uint256 tokenId) internal view {
        require(_exists(tokenId), "MRC998: invalid token ID");
    }
    ...
    function mint(identity to) onlyOwner public returns(uint256) {
        require(to != identity(0), "ti0");
    ...
    function mintFromBatchTokens(identity to, uint256[] memory childTokenIds) public onlyOwner returns(uint256) {
        require(to != identity(0), "ti0");
        uint256 index = _index.current();
        _safeMint(to, index);
